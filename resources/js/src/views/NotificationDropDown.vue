<template>
  ...
  <vs-dropdown vs-custom-content vs-trigger-click class="cursor-pointer">
    <feather-icon icon="BellIcon" class="cursor-pointer mt-1 sm:mr-6 mr-2" :badge="count" />

    <vs-dropdown-menu class="notification-dropdown dropdown-custom vx-navbar-dropdown">

      <div class="notification-top text-center p-5 bg-primary text-white">
        <h3 class="text-white">{{ count }} New</h3>
        ...
        <ul class="bordered-items">
          <li v-for="ntf in notifications" :key="ntf.index" class="flex justify-between px-4 py-4 notification cursor-pointer">
            <div class="flex items-start">
              ...
              <div class="mx-2">
                <span class="font-medium block notification-title" :class="[`text-${ntf.type}`]">Product changes</span>
                <small>{{ ntf.data }}</small>
              </div>
            </div>
    ...
    randomDate({ hr, min, sec }) {
      ...
      loadNotifications(){
          return new Promise((resolve, reject) => {
              axios.get("/api/products/notifications")
                  .then((response) => {
